...
async function register(params: any, origin: string) {
    ...
    try {
        ...
        if (process.env.SHOW_VERIFICATION_TOKEN !== 'true') {
            throw err;
        }
        console.warn(`Verification email failed for ${account.email}:`, err);
    }

    return {
        verificationToken: process.env.SHOW_VERIFICATION_TOKEN === 'true' ? account.verificationToken : undefined
    };
}
